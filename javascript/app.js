...
const correctQuiz = (event, form, answerList) => {
  const answerInputs = form.querySelectorAll("input");
  const uniqueInputs = [];
...
const keyCodeQuizAnswers = ['37', '38', '39', '40'];
const quizForm = document.getElementById('fonction-quiz');
$('#fonction-quiz-submit').click(() => correctQuiz(event, quizForm, keyCodeQuizAnswers));

const directionsQuizAnswers = ['(1,0)', '(0,-1)', '(-1,0)'];
const directionsForm = document.getElementById('directions-quiz');
$('#directions-quiz-submit').click(() => correctQuiz(event, directionsForm, directionsQuizAnswers));
